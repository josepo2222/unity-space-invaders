...
public class GeneradorAliens : MonoBehaviour
{
    ...
    public Rigidbody2D prefabnalien;
	...
	void generarAliens (int filas, int columnas, float espacioH, float espacioV, float escala = 1.0f)
	{
      ...
      /*  Vector2 posicion = new Vector2(0 + (espacioH * j), 3 + (espacioV * i));
        Rigidbody2D alien = (Rigidbody2D)Instantiate(prefabnalien, posicion, transform.rotation);
        */
    }
